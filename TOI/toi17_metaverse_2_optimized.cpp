...
int m,n,k,p;
int Wr[N],Wc[N],Or[N],Oc[N],Wci[N],Oci[N],dp[2][N];
vector<int> vr,vc;
...
int main(){
    ...
    for(int i=1;i<=k;i++){
        ...
        vc.emplace_back(Wc[i]);
        vc.emplace_back(Oc[i]);
    ...
    for(int i=2;i<=p;i++){
        ...
        for(int l=1;l<=sr;l++){
            for(auto j:O[l]){
                seg1.update(1,sc,1,Oci[j],dp[i&1^1][j]-Or[j]-Oc[j]);
                seg2.update(1,sc,1,Oci[j],dp[i&1^1][j]-Or[j]+Oc[j]);
            }
            for(auto j:W[l]){

...
poly exp(const poly &A,int n){
	...
	for (int x = 1; x < n; x *= 2) {
		IB = 2*IB-rsz(B*IB*IB,x);
		poly Q = deriv(rsz(A,x)); Q += rsz(IB*(deriv(B)-B*Q),2*x-1); 
		/// first x-1 terms of dif(B)-conv(B,Q) are zero
		B = B+rsz(B*(rsz(A,2*x)-integ(Q)),2*x); 
...
int main(){
    ...
    poly b=exp(exp(poly{0,1},n+1)-poly{1},n+1);
    for(int i=0;i<=n;i++)cout << b[i]*fac[i] << " ";
}
...
struct Node{
    ll spare,need;
    Node():spare(0),need(0){}
    Node(ll x,ll y):spare(x),need(y){}
    void apply(int l,int r,const Tag &v){

    }
    friend Node operator+(const Node &lhs,const Node &rhs){
        ll used=min(lhs.spare,rhs.need);
        return Node(lhs.spare+rhs.spare-used,lhs.need+rhs.need-used);
    }
};
...
int main(){
    ...
    auto upd=[&](int i){
        ll d=a[i]-b[i];
        if(d>0)s.modify(i,Node(d,0));
        else s.modify(i,Node(0,-d));
    };

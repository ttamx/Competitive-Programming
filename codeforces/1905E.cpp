...
map<ll,tuple<ll,mint,mint>> dp;

pair<mint,mint> solve(ll i,ll n){
    if(n==0)return {0,0};
    if(n==1)return {i,1};
    auto it=dp.find(n);
    if(it!=dp.end()){
        auto [j,val,mul]=it->second;
        return {val+(i-j)*mul,mul};
    }
    ll cr=n/2,cl=n-cr;
    auto [vall,mull]=solve(i*2,cl);
    auto [valr,mulr]=solve(i*2+1,cr);
    mint mul=(binpow(mint(2),cl)-1)*(binpow(mint(2),cr)-1),val=mul*i;
    val+=vall+valr;
    mul+=2*(mull+mulr);
    dp[n]={i,val,mul};
    return {val,mul};

...
FPS work(vector<mint> a,int k){
    function<FPS(int,int)> rec=[&](int l,int r){
        if(l==r)return FPS{mint(1),-a[l]};
        int m=(l+r)/2;
        return rec(l,m)*rec(m+1,r);
    };
    FPS f=rec(0,a.size()-1).inv(k+1).log(k+1).diff();
    f.insert(f.begin(),mint(a.size()));
    for(int i=0;i<=k;i++){
        f[i]*=comb.ifac(i);
    }
    return f;
...
void runcase(){
    ...
    NTT<mint>::ntt(f);
    mint inv=mint(m).inv();
    for(int i=0;i<m;i++)g[-i&(m-1)]=binpow(f[i],t)*inv;
    NTT<mint>::ntt(g);
    mint ans=g[k];
    if(k!=0&&k!=(1<<n))ans+=g[m-k];
    cout << ans/binpow(mint(2),t) << "\n";
}
